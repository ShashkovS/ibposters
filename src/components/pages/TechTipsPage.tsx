...
import TechTipPoster from '../posters/tech-tips/TechTipPoster';
...
const TechTipsPage: React.FC = () => {
    const handlePrint = () => {
        window.print();
    };

    // Add a class to the body for portrait printing and remove it on cleanup
    React.useEffect(() => {
        document.body.classList.add('print-a3-portrait');
        return () => {
            document.body.classList.remove('print-a3-portrait');
        };
    }, []);
    ...
    return (
        <div className="bg-gray-100 min-h-screen">
            <div className="p-4 bg-white shadow-md no-print flex justify-between items-center sticky top-0 z-10">
                ...
                <h1 className="text-lg font-bold font-roboto-slab text-gray-700 hidden md:block">macOS & Terminal Productivity Posters</h1>
                <button
                    onClick={handlePrint}
                    className="flex items-center bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 transition-colors"
                >
                    <PrintIcon />
                    Print All (A3 Portrait)
                </button>
            </div>
